function [df_interest]=estimation_mcculloch_weighted_all(price, coupon_all, dtm, cdspremia, cds_initial, K, T, boot, num_boot, weights_sqrt)
...
for t=1:T
   ...
   % Find NaN price values and remove
    ...
    for jj=1:num_boot
        ...
        WEIGHT_SQRT=WEIGHT_SQRT(:,indexes(:,jj));
        
        % Estimated bootstrapped discount factor    
%         beta_boot= lscov(X,z,WEIGHT_SQRT');
        beta_boot= inv(X_boot'*diag(WEIGHT_SQRT)*X_boot)*(X_boot'*diag(WEIGHT_SQRT)*z_boot);
        % Estimated bootstrapped discount factor
